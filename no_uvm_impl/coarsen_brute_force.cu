...
Hypergraph* coarsen_no_uvm_brute_force(
                            Hypergraph* hgr, int iter, int LIMIT, float& time, float& other_time, OptionConfigs& optcfgs, 
...
# if 0
    ...
    if (optcfgs.enableSelection) {
        ...
        if (optcfgs.filename.find("af_4_k101") == std::string::npos) {
            if (hgr->sdHedgeSize / hgr->avgHedgeSize > 5) {
                optcfgs.optForKernel4 = true;
                optcfgs.optForKernel6 = true;
            } else if (hgr->avgHedgeSize > 75) {
                optcfgs.optForKernel4 = true;
                optcfgs.optForKernel6 = true;
            }

            if (hgr->avgHedgeSize >= 75) {
                optcfgs.useNewKernel12 = true;
                optcfgs.useNewParalOnBaseK12 = 0;
                optcfgs.sortHedgeLists = true;
                optcfgs.useNewKernel14 = true;
                optcfgs.useNewParalOnBaseK14 = 0;
            } else {
        ...
        std::cout << __LINE__ << ":"
                  ...
                  << "pattern for K4K6," << optcfgs.optForKernel4 << "," << optcfgs.optForKernel6 << "\n"
                  << "pattern for K12," << optcfgs.useNewKernel12 << "," << optcfgs.useNewParalOnBaseK12 << "\n"
    ...
    if (!optcfgs.useNewKernel1_2_3) {
        ...
        // std::cout << "det parallel matching policy time:" << time1 + time2 + time3 << "\n";
    } else {
        TIMERSTART(1)
        ...
        if (!optcfgs.useNewParalOnBaseK12) {
            ...
            if (!optcfgs.useMemOpt) {
                ...
                markDuplicateParentInPins1<<<hgr->hedgeNum, blocksize>>>(hgr->hedges, hgr->nodes, hgr->adj_list, aux->isDuplica, hgr->hedgeNum, hgr->nodeNum, 
                                                coarsenHgr->hedges, coarsenHgr->hedgeNum, aux->d_totalPinSize, aux->eNextId, aux->eInBag, 
                                                aux->adj_node_parList, hgr->maxDegree);
                // markDuplicateParentInPins1_mod<<<hgr->hedgeNum, blocksize>>>(hgr->hedges, hgr->nodes, hgr->adj_list, aux->isDuplica, hgr->hedgeNum, hgr->nodeNum, 
                //                                 coarsenHgr->hedges, coarsenHgr->hedgeNum, aux->d_totalPinSize, aux->eNextId, aux->eInBag, 
                //                                 aux->adj_node_parList, hgr->maxDegree, 1000, 2);
                // markDuplicateParentInPins1_workstealing<<<hgr->hedgeNum, blocksize>>>(hgr->hedges, hgr->nodes, hgr->adj_list, aux->isDuplica, hgr->hedgeNum, hgr->nodeNum, 
                //                                 coarsenHgr->hedges, coarsenHgr->hedgeNum, aux->d_totalPinSize, aux->eNextId, aux->eInBag, 
                //                                 aux->adj_node_parList, hgr->maxDegree, 1000, 3);
                ...
                // int* d_off;
                // CHECK_ERROR(cudaMalloc((void**)&d_off, (hgr->hedgeNum + 1) * sizeof(int)));
                // CHECK_ERROR(cudaMemcpy((void *)d_off, hgr->hedges + E_OFFSET1(hgr->hedgeNum), hgr->hedgeNum * sizeof(int), cudaMemcpyDeviceToDevice));
                // CHECK_ERROR(cudaMemcpy((void *)&d_off[hgr->hedgeNum], &hgr->totalEdgeDegree, sizeof(int), cudaMemcpyHostToDevice));
                // cub::DeviceSegmentedSort::SortKeys(d_temp_storage, temp_storage_bytes, aux->adj_node_parList, aux->adj_node_parList, hgr->totalEdgeDegree, hgr->hedgeNum, d_off, d_off+1);
                // std::cout << "temp_storage_in_GB: " << temp_storage_bytes / (1024.f * 1024.f * 1024.f) << " GB.\n";
                // CHECK_ERROR(cudaMalloc((void**)&d_temp_storage, temp_storage_bytes));
                // cudaStream_t stream;
                // cudaStreamCreateWithFlags(&stream, cudaStreamDefault);
                // cub::DeviceSegmentedSort::SortKeys(d_temp_storage, temp_storage_bytes, aux->adj_node_parList, aux->adj_node_parList, hgr->totalEdgeDegree, hgr->hedgeNum, d_off, d_off+1, stream);
                // CHECK_ERROR(cudaStreamSynchronize(stream));
                // CHECK_ERROR(cudaStreamDestroy(stream));
                // CHECK_ERROR(cudaFree(d_temp_storage));
                // CHECK_ERROR(cudaFree(d_off));
                // TIMERSTOP(_sort)
                // markDuplicateParentInPins1_binsearch<<<num_blocks, blocksize>>>(hgr->hedges, hgr->nodes, hgr->adj_list, aux->isDuplica, hgr->hedgeNum, hgr->nodeNum, 
                //                                 coarsenHgr->hedges, coarsenHgr->hedgeNum, aux->d_totalPinSize, aux->eNextId, aux->eInBag, 
                //                                 aux->adj_node_parList, aux->pins_hedgeid_list, hgr->totalEdgeDegree, hgr->maxDegree);
                } else {
