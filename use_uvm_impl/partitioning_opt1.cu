...
void partition_opt1(Hypergraph* hgr, unsigned int K, bool use_curr_precision, float& time) {
    ...
    if (static_cast<long>(zeroPartWeight) > nonzeroPartWeight) {
        ...
        while (1) {
            ...
            CHECK_ERROR(cudaMallocManaged(&move_flag, hgr->nodeNum * sizeof(int)));
            for (int i = 0; i < count[0]; ++i) {
                move_flag[nodeListnz[i].nodeid - hgr->hedgeNum] = 1;
            }
            TIMERSTART(_tmp)
            reComputeGain<<<UP_DIV(count[0], blocksize), blocksize>>>(hgr, nodeListnz, count, move_flag);
            thrust::sort(thrust::device, nodeListnz, nodeListnz + count[0], mycmp2());
            TIMERSTOP(_tmp)
            if (ccount == 0) {
                std::ofstream debug("../debug/initial_partition_nodelist_info.txt");
                for (int i = 0; i < count[0]; ++i) {
