...
static cll::opt<unsigned> refiter(cll::Positional,
                                  ...
                                  cll::init(2));
static cll::opt<unsigned> numPartitions(
                                        // cll::Positional,
                                        "numPartitions",
...
int main(int argc, char** argv) {
  ...
  std::cout << "\n==================== " << numPartitions << " way partitioning starting perform...\n";
  printf("ceil(log2(n)) == floor(log2(n)) ? %d\n", (int)ceil(log2(numPartitions)) == (int)floor(log2(numPartitions)));

  printf("after first round of bipartitioning...\n");
  ...
  for (unsigned i = 0; i < numPartitions; i++) {
    std::cout << "part " << i << " size " << parts[i].size() << "\n";
  }
  ...
  for (int level = 0; level < num; level++) {
    ...
    for (int i = 0; i < k; i++) {
      printf("part %d size %lu\n", i, nodesvec[i].size());
    }

    std::vector<std::vector<GNode>> hedgevec(k);
    std::ofstream debug("/home/wuzhenlin/workspace/gHyPart/scripts/bipart.txt");
    ...
    for (unsigned i : toProcess) {
      ...
      if (kValue[i] > 1) {
        ...
        galois::do_all(
            ...
            [&](GNode h) {
              // debug << "hedge " << h << " " << graph.getData(h).index << "\n";
              // debug << "hedge " << graph.getData(h).index << ": ";
        ...
        std::cout << "Edge Cut," << computingCut(gr) << "\n";
        ...
        printf("subgraph partitioning done, nodesvec[i].size()=%d, i=%d\n", (int)nodesvec[i].size(), i);
        int count0 = 0;
        int count1 = 0;
        galois::do_all(
  ...
  execTime.stop();
  middle_time += (execTime.get() / 1000.0f); 
#endif

  std::cout << "Coarsening time(s):," << ctime << "\n";
  std::cout << "Partitiong time(s):," << ptime << "\n";
  std::cout << "Refinement time(s):," << rtime << "\n";
  std::cout << "Middle time(s):," << middle_time << "\n";
  std::cout << "Coarsening time1(s):," << Ctime << "\n";
  std::cout << "Partitiong time1(s):," << Ptime << "\n";
  std::cout << "Refinement time1(s):," << Rtime << "\n";
  std::cout << "Total time(s):," << ctime + ptime + rtime + middle_time << "\n";
  std::cout << "Final Edge Cut," << computingCut(graph) << "\n\n";
