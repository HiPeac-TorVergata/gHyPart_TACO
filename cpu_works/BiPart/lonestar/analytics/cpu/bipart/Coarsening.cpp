...
void parallelPrioRand(MetisGraph* graph, int iter) {
  ...
  galois::do_all(
      ...
      [&](GNode item) {
        ...
        // debug << fineGGraph->getData(item).netrand << " " << fineGGraph->getData(item).netval << std::endl;
        // debug << "hedge " << item << ": ";
  ...
  if (iter == 0) {
    ...
    //     galois::iterate(size_t{0}, fineGGraph->hedges),
    //     [&](GNode item) {
    //         // std::cout << "hedge: " << item << ", netval: " << fineGGraph->getData(item).netval
    //         //           << ", netrand: " << fineGGraph->getData(item).netrand << ", idNum: " << fineGGraph->getData(item).netnum << "\n";
    //         // debug << "hedge " << item << ": ";
    //         debug << fineGGraph->getData(item).netrand << " " << fineGGraph->getData(item).netval << std::endl;
    //         // for (auto c : fineGGraph->edges(item)) {
    //         //     auto dst = fineGGraph->getEdgeDst(c);
    //         //     // debug << dst << " ";
    //         // //     std::cout << "node: " << dst-fineGGraph->hedges+1 << ", netval: " << fineGGraph->getData(dst).netval
    //         // //               << ", netrand: " << fineGGraph->getData(dst).netrand << ", hedgeID: " << fineGGraph->getData(dst).netnum << "\n";
    //         // }
    //         // debug << "\n";
    //     },
    //     galois::steal(), galois::loopname("finer graph results"));
  }
...
MetisGraph* coarsen(MetisGraph* fineMetisGraph, unsigned coarsenTo,
  ...
  while (Size > coarsenTo) {
    ...
    // is "
    //          << hedgeSize << "\n";
    printf("coarsen level %d, hedges %d, nodes %d\n", iterNum, hedgeSize, Size);
    if (hedgeSize < 1000)
      break;
