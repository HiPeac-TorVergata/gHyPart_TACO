...
void parallel_refine_KF(GGraph& g, float, unsigned refineTo) {
  ...
  printf("nodeSize: %u accum: %u\n", nodeSize.reduce(), accum.reduce());
  unsigned pass = 0;
  // std::cout<<"cut parallel "<<calculate_cutsize(g)<<"\n";
  // initGain(g);
  while (pass < refineTo) {
    // if (pass == 1 && accum.reduce() == 2901) {
    //   std::ofstream debug("/home/wuzhenlin/workspace/gHyPart/scripts/bipart.txt");
    //   for (GNode n = g.hedges; n < g.size(); n++) {
    //     debug << g.getData(n).FS << " " << g.getData(n).TE << " " << g.getData(n).counter << "\n";
    //   }
    // }
    // T.start();
    ...
    // if (pass == 0 && accum.reduce() == 2901) {
    //   std::ofstream debug("/home/wuzhenlin/workspace/gHyPart/scripts/bipart.txt");
    //   for (GNode n = g.hedges; n < g.size(); n++) {
    //     debug << g.getData(n).FS << " " << g.getData(n).TE << " " 
    //           << g.getData(n).counter << " " << g.getData(n).getPart() << "\n";
    //   }
    // }
    GNodeBag nodelistz;
    GNodeBag nodelisto;
    ...
    printf("zeroW: %u oneW: %u none: %d negative: %d\n", zeroW, oneW, none, negative);
    ...
    if (zeroW <= oneW) {
      ...
      galois::do_all(
          ...
          [&](GNode n) {
            ...
            g.getData(n).counter++;
            // if (pass == 0 && accum.reduce() == 2901) {
            //   printf("nodeid %u counter %u\n", (int)(n - g.hedges), g.getData(n).counter);
            // }
          },
          galois::loopname("swap"));
    }
  ...
  std::vector<std::vector<uint64_t>> parts(2);
  for (GNode n = g.hedges; n < g.size(); n++) {
    unsigned p = g.getData(n).getPart();
    parts[p].push_back(n - g.hedges + 1);
  }
